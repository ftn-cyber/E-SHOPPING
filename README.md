...
<div class="container">
    <div class="auth-box" id="formLogin">
        ...
        <form onsubmit="cekLogin(event)">
            ...
            <p style="margin-top:10px;font-size:12px;color:#666">💡 Petunjuk: Cari celahnya ya...</p>
        </form>
    </div>
...
// ✅ SISTEM LOGIN SUDAH DIPERBAIKI - PASTI BERHASIL
function cekLogin(e){
    e.preventDefault();
    const user = document.getElementById('user').value.trim();
    const pass = document.getElementById('pass').value.trim();

    // SYARAT SESUAI PERMINTAAN: Username yang diminta + sandi apa saja
    if(user == "root@e-shopping.com'or 1=1 --+" && pass.length > 0){
        bukaAdmin(); // LANGSUNG MASUK JIKA SESUAI
    }else{
        alert("❌ Gagal masuk! Masukkan username yang benar dan sandi apa saja.");
    }
}
